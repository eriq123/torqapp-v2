	...
	<form action="{{route('section.request_crud')}}" method="POST" enctype="multipart/form-data">
			...
			<div class="panel-body col-md-8 col-md-offset-2" style="padding: 0 0 0 0;">
				...
				<div class="row">
					<div class="col-md-4">
						<select name="director_id" class = "form-control{{ $errors->has('director_id') ? ' is-invalid' : '' }}" required class="form-control" id = "director_id">
							...
							<option value="{{$director->id}}" {{($director->id == $req->campus_director['id']) ? 'selected' : '' }}>{{ $director->full_name }}</option>
							@endforeach
						</select>
						<input type="text" name="director_role" class="form-control" value="{{$req->campus_director['role']}}">
					</div>
				</div>
				<br>
				<div class="row">
					<div class="col-md-6 col-md-offset-3">
						<div class="input-group">
							...
							<select name="adaa_id" class = "form-control{{ $errors->has('adaa_id') ? ' is-invalid' : '' }}" required class="form-control" id = "adaa_id" style="position: static!important;">
								...
								<option value="{{$adaa->id}}" {{($adaa->id == $req->adaa['id']) ? 'selected' : '' }}>{{ $adaa->full_name }}</option>
								@endforeach
							</select>
						</div>
						<input type="text" name="adaa_role" class="form-control" value="{{$req->adaa['role']}}">
					</div>
				</div>
				<br>
				...
				<div class="row">
					<div class="col-md-6">
						...
						<input type="text" name="sh_name" class="form-control" value="{{$req->section_head['name']}}">
						<input type="text" name="sh_role" class="form-control" value="{{$req->section_head['role']}}">
						<br>
						<select name="department_id" class = "form-control{{ $errors->has('department_id') ? ' is-invalid' : '' }}"  class="form-control" id = "department_id">
							...
							<option value="{{$department->id}}" {{($department->id == $req->department_head['id']) ? 'selected' : '' }}>{{ $department->full_name }}</option>
							@endforeach
						</select>
						<input type="text" name="department_role" class="form-control" value="{{$req->department_head['role']}}">
					</div>
				</div>
				<br>
			</div>
		...
		<div class="row">
			<div class="panel-body col-md-6 col-md-offset-3">
				...
				<a data-toggle="modal" data-target="#EditModal" h6ref="#" role="button" class="mb-xs btn btn-dark"><span class = "fa fa-list">&nbsp;</span>Request Items</a>

				<input type="hidden" name="req_id" id="req_id" value="{{$req->id}}">
				<input type="hidden" name="Rcontent" id="Rcontent" value="{{$req->content}}">
				<input type="hidden" name="old_items" id="old_items" value="{{$item_ids}}">
				<input type="hidden" name="items" id="items" value="{{$item_ids}}">
				<input type="hidden" name="app_id" id="app_id" value="{{$app->id}}">
				<button class="mb-xs btn btn-success" type="submit" id = "submit" name="submit" value="update"><span class = "fa fa-send">&nbsp;</span>Update</button>
			</div>
		</div>
		<br>
	</form>
...
<script>
	$(document).ready(function(){
		...
		CKEDITOR.instances['editor'].setData($('#Rcontent').val());
		$('#id').val($('#items').val());
					...
					<table id="Table1" class="table table-hover " style="width:100%;">
						...
						<tbody>
							@foreach($app->item as $item)
								...
								@else
									@if( in_array("$item->id",explode (",", $item_ids) ) ) 
									<tr>
										<td>
											<input type="checkbox" class="child" value="{{$item->id}}" @if( in_array("$item->id",explode (",", $item_ids) ) ) checked="checked" @endif>
										</td>
										<td>{{$item->code}}</td>
										<td>{{$item->category}}</td>
										<td>{{$item->description}}</td>
